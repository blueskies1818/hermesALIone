...
const hermesAPI = {
  ...
  onTtsAudio: (callback: (base64Chunk: string) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, base64Chunk: string): void =>
      callback(base64Chunk);
    ipcRenderer.on("chat-tts-audio", handler);
  ...
  getFullConfig: (): Promise<Record<string, unknown>> =>
    ipcRenderer.invoke("get-full-config"),
  saveFullConfig: (config: Record<string, unknown>): Promise<void> =>
    ipcRenderer.invoke("save-full-config", config),
  getConfigSchema: (): Promise<{
    fields: Record<string, Record<string, unknown>>;
    category_order: string[];
  }> => ipcRenderer.invoke("get-config-schema"),
  getConfigDefaults: (): Promise<Record<string, unknown>> =>
    ipcRenderer.invoke("get-config-defaults"),
  getConfigRaw: (): Promise<string> => ipcRenderer.invoke("get-config-raw"),
  saveConfigRaw: (yamlText: string): Promise<void> =>
    ipcRenderer.invoke("save-config-raw", yamlText),
  restartGatewayForConfig: (): Promise<boolean> =>
    ipcRenderer.invoke("restart-gateway-for-config"),

  // Plugins
  getPluginsHub: (): Promise<{
    plugins: Array<{
      name: string;
      version: string;
      description: string;
  ...
  }> => ipcRenderer.invoke("get-plugins-hub"),
  installPlugin: (
    identifier: string,
    force?: boolean,
    enable?: boolean,
    ...
    missing_env?: string[];
    error?: string;
  }> => ipcRenderer.invoke("install-plugin", identifier, force, enable),
  enablePlugin: (name: string): Promise<boolean> =>
    ipcRenderer.invoke("enable-plugin", name),
  disablePlugin: (name: string): Promise<boolean> =>
    ipcRenderer.invoke("disable-plugin", name),
  updatePlugin: (name: string): Promise<boolean> =>
    ipcRenderer.invoke("update-plugin", name),
  ...
  savePluginProviders: (
    ...
    ipcRenderer.invoke("save-plugin-providers", memoryProvider, contextEngine),
  setPluginVisibility: (name: string, hidden: boolean): Promise<boolean> =>
    ipcRenderer.invoke("set-plugin-visibility", name, hidden),
  ...
  vault: {
    ...
    > => ipcRenderer.invoke("vault:list-buckets"),
    ...
    > => ipcRenderer.invoke("vault:browse", bucketId, path),
    ...
    > => ipcRenderer.invoke("vault:search", query, bucketId, limit, tokenBudget, resultDepth),
    ...
    createBucket: (
      ...
      ipcRenderer.invoke("vault:create-bucket", name, description, customPath),

    deleteBucket: (bucketId: string): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:delete-bucket", bucketId),
    ...
    updateBucket: (
      ...
      ipcRenderer.invoke("vault:update-bucket", bucketId, name, description),
    ...
    }> => ipcRenderer.invoke("vault:reindex", bucketId, force),

    tree: (
      bucketId: string,
    ): Promise<{
      ok: boolean;
      tree: Array<{ name: string; relPath: string; fullPath: string; type: "file" | "dir"; children?: any[] }>;
      bucketPath: string;
      error?: string;
    }> => ipcRenderer.invoke("vault:tree", bucketId),

    readFile: (fullPath: string): Promise<{ ok: boolean; content: string; error?: string }> =>
      ipcRenderer.invoke("vault:read-file", fullPath),

    writeFile: (
      fullPath: string,
      content: string,
    ): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:write-file", fullPath, content),

    moveItem: (
      fromPath: string,
      toDir: string,
    ): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:move-item", fromPath, toDir),

    createFile: (fullPath: string): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:create-file", fullPath),

    createFolder: (fullPath: string): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:create-folder", fullPath),

    deleteItem: (
      fullPath: string,
      isDir: boolean,
    ): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke("vault:delete-item", fullPath, isDir),

    getLinks: (
      bucketId: string,
    ): Promise<{ ok: boolean; links: Array<{ fromPath: string; toPath: string | null; toTitle: string }>; error?: string }> =>
      ipcRenderer.invoke("vault:get-links", bucketId),
  },
};
