...
interface HermesAPI {
  ...
  vault: {
    ...
    moveItem: (fromPath: string, toDir: string) => Promise<{ ok: boolean; error?: string }>;
    createFile: (fullPath: string) => Promise<{ ok: boolean; error?: string }>;
    createFolder: (fullPath: string) => Promise<{ ok: boolean; error?: string }>;
    deleteItem: (fullPath: string, isDir: boolean) => Promise<{ ok: boolean; error?: string }>;
    getLinks: (bucketId: string) => Promise<{ ok: boolean; links: Array<{ fromPath: string; toPath: string | null; toTitle: string }>; error?: string }>;
  };
}
