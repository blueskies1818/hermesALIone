...
class HermesCLI:
    ...
    def _browser_control_approval_callback(self, action: str, args: dict, summary: str) -> str:
        """Adapt the generic approval UI for browser_control.

        The browser_control handler expects verdicts of the form
        `approve_once` | `approve_session` | `always_approve` | `deny`.
        """
        verdict = self._approval_callback(
            command=f"browser_control: {summary}",
            description=f"Allow browser_control to perform `{action}`?",
        )
        return {
            "once": "approve_once",
    ...
    def run(self):
        ...
        # Browser-control shares the same approval UI.
        try:
            from tools.browser_control.tool import set_approval_callback as _set_bc_cb
            _set_bc_cb(self._browser_control_approval_callback)
        except ImportError:
            pass
