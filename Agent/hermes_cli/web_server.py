...
# CORS: restrict to localhost origins by default.  When --insecure is passed
# (app.state.allow_public = True), relax to any origin so remote Electron
# clients and mobile browsers can reach the dashboard.

@app.middleware("http")
async def cors_middleware(request: Request, call_next):
    """Dynamic CORS — restrictive on loopback, permissive with --insecure."""
...
def _is_loopback_client(request: Request) -> bool:
    """True if the request originates from a loopback interface.

    Trusted without a session token so the Electron main process can call
    the API directly in local mode without having to scrape the token out
    of the SPA index page.
    """
...
def _has_valid_session_token(request: Request) -> bool:
    """True if the request carries a valid dashboard session token or API key.
    ...
    Also accepts the persistent API_SERVER_KEY from .env so remote Desktop
    clients can authenticate with the same key they use for the gateway.
    """
    ...
    # Persistent API server key (for remote Desktop clients)
    api_key = os.getenv("API_SERVER_KEY", "").strip()
    if api_key:
        expected_key = f"Bearer {api_key}"
        if hmac.compare_digest(auth.encode(), expected_key.encode()):
            return True
...
async def host_header_middleware(request: Request, call_next):
    ...
    """
    # In --insecure mode, skip Host header validation — the operator has
    # opted into accepting connections from any origin.
...
async def auth_middleware(request: Request, call_next):
    """Require auth on /api/ routes except the public list.

    Loopback clients (127.0.0.1, ::1, localhost) are trusted without a token
    so the Electron main process can call the API directly in local mode.
    Remote clients must present the session token or API_SERVER_KEY.
    """
    path = request.url.path
    if path.startswith("/api/") and path not in _PUBLIC_API_PATHS:
        if not _is_loopback_client(request) and not _has_valid_session_token(request):
            return JSONResponse(
                status_code=401,
...
async def get_health():
    ...
    Used by remote Electron clients and SSH tunnel probes to verify
    the API server is reachable before attempting authenticated calls.
    """
    return {
        "status": "ok",
        "version": __version__,
...
async def get_version():
    """Return the Hermes Agent version — no auth required."""
    return {
        "version": __version__,
        "release_date": __release_date__,
    }
...
async def install_skill(body: SkillInstall):
    """Install a skill by identifier or URL."""
    from hermes_cli.skills_hub import do_install
    try:
        do_install(body.identifier, force=body.force, skip_confirm=True)
        return {"ok": True}
    except Exception as e:
        _log.exception("POST /api/skills/install failed")
        raise HTTPException(status_code=500, detail=str(e))
...
async def uninstall_skill(body: SkillUninstall):
    """Uninstall a skill by name."""
    from tools.skills_hub import uninstall_skill as uninstall_skill_fn
    try:
        success, msg = uninstall_skill_fn(body.name)
        if not success:
        ...
        _log.exception("POST /api/skills/uninstall failed")
        raise HTTPException(status_code=500, detail=str(e))
...
# Mount extended API routes (memory, soul, tools, kanban, chat, gateway, etc.).
try:
    from hermes_cli.web_api_extended import router as extended_router
    app.include_router(extended_router)
    _log.info("Mounted extended API routes (web_api_extended)")
except Exception as exc:
...
def start_server(
    ...
    *,
    ...
    # Generate a one-time pairing code for remote client linking when no
    # API_SERVER_KEY is configured. The operator shares this code with the
    # remote user, who enters it on the Connect page to pair.
    from hermes_cli.web_api_extended import generate_pairing_code
    code = generate_pairing_code()
    if code:
        print(f"  Pairing code: {code}")
        print("  Share this code with remote clients to link them to this server.")
        print("  (Expires in 15 minutes; one-time use only.)")
