...
- POST /v1/transcribe              — transcribe audio to text (multipart file upload)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "endpoints": {
                ...
                "transcribe": {"method": "POST", "path": "/v1/transcribe"},
            },
        })
    ...
    async def _handle_transcribe(self, request: "web.Request") -> "web.Response":
        ...
        Accepts a multipart file upload with an audio file, delegates to
        the configured STT provider, and returns the transcription as JSON.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        suffix = ".wav"
        filename = getattr(audio_field, "filename", "")
        if filename:
        ...
        try:
            ...
            return web.json_response(
                {"success": False, "error": "Transcription tools not available"},
                status=500,
            )
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        voice_mode = _coerce_request_bool(body.get("voice_mode"), default=False)
    ...
    async def _write_sse_chat_completion(
        ...
        gateway_session_key: str = None, voice_mode: bool = False,
    ) -> "web.StreamResponse":
        """Write real streaming SSE from agent's stream_delta_callback queue.

        If *voice_mode* is True, text deltas are buffered into sentences and
        synthesised to base64 MP3 audio chunks emitted as
        ``event: hermes.tts.audio`` SSE events alongside the normal text
        deltas.  Tool progress events are never spoken.
        ...
        try:
            ...
            # Voice mode: lazy-import TTS sentence buffer so the import
            ...
            async def _emit(item):
                ...
                When voice_mode is active, text deltas are also buffered
                into sentences and synthesised to TTS audio chunks emitted
                as ``event: hermes.tts.audio``.
                """
